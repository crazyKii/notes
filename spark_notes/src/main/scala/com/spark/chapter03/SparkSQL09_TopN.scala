 ...
 * Desc:  TopN案例实现
 */
object SparkSQL09_TopN {
  def main(args: Array[String]): Unit = {
    //创建配置文件对象
    val conf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("SparkSQL_TopN")
    //创建SparkSession对象
...
class AreaClickUDAF extends UserDefinedAggregateFunction {
  ...
  override def inputSchema: StructType = {
    new StructType(Array[StructField](StructField("city_name",StringType)))
  }

  // 缓存的数据类型  用Map缓存城市以及该城市点击数 ：北京->2,天津->3           总的点击量Long：  北京2 + 天津3 = 5
  override def bufferSchema: StructType = {
    new StructType(Array[StructField](StructField("city_count",MapType(StringType,LongType)),StructField("total_count",LongType)))
  }
